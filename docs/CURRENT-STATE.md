...
- Static pages are built from `src/` into generated `public/`. **Edit `src/`, never
  `public/`.**
...
- `api/_lib/sms-sender.mjs` exists, but the application outbound SMS sender is deliberately
  **dark/unreachable**. No live route imports it and `OUTBOUND_SMS_ENABLED` remains off/unset.
...
- A controlled `/home-value` submission passed the enabled Production path and reached
  HubSpot with communications consent ungranted.

### Communications consent

- `COMMUNICATIONS_CONSENT_ENABLED=true` in Production.
- `/home-value` presents separate optional SMS and AI-voice permission choices, unchecked
...
A controlled real STOP reached Twilio and the deployed Production webhook. Production logs
classified it with `rule=opt_out_type_stop`, which requires Twilio to have supplied
`OptOutType=STOP`. The application then:
...
### Separate Twilio confirmation-delivery incident

The handset did not receive Twilio-generated HELP, STOP, or START confirmations even though:

- Advanced Opt-Out is enabled and configured on the correct Messaging Service;
- STOP supplied `OptOutType=STOP` to the webhook;
- Twilio Messaging Logs showed inbound receipt / TwiML fetch; and
...
> fresh active-block read -> validated human decision -> append `unsuppressed` -> durable
> post-read -> HubSpot clearance projection
...
**Unsuppression never grants consent.** It also does not reconcile Twilio provider-level
opt-out state.
...
- `api/_lib/send-permission.mjs` is bound to the real HubSpot lookup and real Neon
  suppression executor;
...
- the transport performs at most one provider attempt and does not blindly retry ambiguous
  failures;
...
`consent_ledger_sender` role. The role itself had already been verified to execute
`get_suppression_state(text)` while being refused direct ledger reads/writes.

A temporary one-shot Production probe then exercised `lookupDurableSuppression()` from the
deployed Vercel application using a fixed fictional NANP number and returned **HTTP 204**.
The probe accepted no user phone input, returned no suppression state, imported no outbound
sender, and had no SMS capability. It was removed immediately after verification.

This closes the remaining Production application-binding evidence gap for the sender-role
lookup.
...
`main` is Production and is now protected by active repository ruleset **Protect Main**.
The ruleset targets the default branch with no bypass actors and enforces:
...
Required approving reviews remain 0 for the single-maintainer workflow. The required status
check does not require branches to be up to date before merge.

The `test` workflow also runs on pushes to `main`. `npm run verify:live` remains a separate
post-deploy public-site check.
...
When repository prose conflicts with newer external evidence, record the correction and
